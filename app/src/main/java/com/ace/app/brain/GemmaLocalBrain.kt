...
class GemmaLocalBrain : LocalBrain {
    ...
    override suspend fun interpretGoal(
        ...
        val prompt = buildString {
            ...
            append("2. clarificationRequired must be true if the user's goal lacks necessary target entities, recipient, object, destination, or required constraint to act, or if it is ambiguous (e.g. 'find me the latest', 'send this', 'book it'). Otherwise false.\n")
            append("3. If clarificationRequired is true, clarificationQuestion must be a concise, helpful natural question asking for the missing detail. If false, clarificationQuestion must be null.\n")
